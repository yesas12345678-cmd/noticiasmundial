...
async function main() {
  ...
  try {
    console.log("=== INICIANDO PIPELINE DIARIO COMPLETO (5 ARTÍCULOS) ===");

    // Determine target date (today or tomorrow)
    const targetDate = new Date();
    // If it's late in the day (past 8:00 PM), schedule for tomorrow instead
    if (targetDate.getHours() >= 20) {
      targetDate.setDate(targetDate.getDate() + 1);
    }
    const dateYMD = targetDate.toISOString().slice(0, 10); // YYYY-MM-DD
    console.log(`Target Publication Date for Scheduled pipeline: ${dateYMD}`);
    ...
    // Fetch up to 5 empty articles to satisfy the pipeline
    const { rows: emptyArticles } = await client.query(
      `SELECT id, title, keyword, category, excerpt, published_at FROM articles WHERE content = '' OR content IS NULL OR length(content) = 0 ORDER BY id ASC LIMIT 5`
    );

    const articlesToGenerate = [...emptyArticles];

    // If less than 5 empty articles, generate new concepts to fill the slots
    while (articlesToGenerate.length < 5) {
      ...
      console.log(`Generating a new concept to fill the pipeline slots (Current size: ${articlesToGenerate.length})...`);
      const conceptPrompt = `
        ...
        Nota: Asegúrate de que las noticias parezcan de actualidad en el contexto global. No incluyas explicaciones, solo el JSON puro.
      `;
      ...
      try {
        const response = await fetch('https://api.deepseek.com/chat/completions', {
          ...
          body: JSON.stringify({
            model: 'deepseek-chat',
            messages: [
              { role: 'system', content: 'Eres un periodista internacional experto y estratega de contenido SEO. Responde únicamente con el JSON solicitado.' },
        ...
        if (!response.ok) throw new Error(`Concept generation failed: ${response.statusText}`);
      ...
      if (conceptData && conceptData.title && conceptData.excerpt) {
        ...
        articlesToGenerate.push(newInserted[0]);
      }
    }

    console.log(`Pipeline initialized with ${articlesToGenerate.length} articles to schedule for ${dateYMD}.`);
    ...
    for (let i = 0; i < articlesToGenerate.length; i++) {
      ...
      console.log(`\n[${i + 1}/5] Processing scheduled article ID ${article.id}: "${article.title}"`);
      console.log(`Scheduled publication time: ${scheduledPublishTime}`);
      console.log(`Target format: ${config.type} (${config.wordMin}-${config.wordMax} words)`);
...
## OBJETIVO OPERATIVO DE ESTE ARTÍCULO (Hora Programada: ${config.hour}:00)
...
`;
        ...
        console.log(`Updating database for ID ${article.id} with scheduled time ${scheduledPublishTime}...`);
        await client.query(
          `UPDATE articles 
           SET title = $1, meta_title = $2, meta_description = $3, excerpt = $4, content = $5, image_url = $6, published_at = $7 
           WHERE id = $8`,
          [
            parsed.title,
            parsed.meta_title,
        ...
        console.log(`Article ID ${article.id} successfully scheduled and updated in database!`);
      }
