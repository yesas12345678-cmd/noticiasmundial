...
// 3. Connect to database and process exactly 1 article
async function run() {
  // Check for random delay flag
  const noDelay = process.argv.includes('--no-delay') || process.env.NO_DELAY === 'true';
  if (!noDelay) {
    // Random delay between 0 and 5 hours (in milliseconds)
    const delayHours = Math.random() * 5;
    console.log(`Introduciendo un retraso aleatorio de ${delayHours.toFixed(2)} horas para simular una publicación orgánica...`);
    await new Promise(resolve => setTimeout(resolve, delayHours * 60 * 60 * 1000));
  } else {
    console.log('Ejecutando inmediatamente (--no-delay detectado).');
  }
